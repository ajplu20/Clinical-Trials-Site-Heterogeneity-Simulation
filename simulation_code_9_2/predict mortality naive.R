predict_mortality_from_data <- function(df, min_mortality = 0.25, max_mortality = 0.75, individual_sd = 0.01) {
  ...
  predictions <- rnorm(n = nrow(df), mean = means, sd = individual_sd)
...
# Example dataframe
#df <- data.frame(site =rep("A",10000))

# Apply function
#df$mortality_probability <- predict_mortality_from_data(df)
#sd(df$mortality_probability)
