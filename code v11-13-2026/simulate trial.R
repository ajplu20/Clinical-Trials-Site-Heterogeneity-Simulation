...
simulate_trial <- function(
  ...
  ## ---- CHANGED: global randomization, ignoring site ----
  # Weighted overall treatment proportion (by planned site sizes)
  ...
  chosen <- sample.int(n_total, n_experiment)
  experiment_vector <- integer(n_total)
...
simulate_multiple_trials <- function(
  ...
  for (i in seq_len(iterations)) {
    ...
    results$naive_t_test[i]           <- perform_naive_t_test(trial_data, alpha = alpha, trial_type = tt, ni_margin = ni_margin)
    results$fix_effect_model_test[i]  <- perform_fix_effect_model_test(trial_data, alpha = alpha, trial_type = tt, basket = FALSE, ni_margin = ni_margin)
    results$mixed_effect_model_test[i]<- perform_mixed_effect_model_test(trial_data, alpha = alpha, trial_type = tt, basket = FALSE, ni_margin = ni_margin)
    results$mantel_haenszel_test[i]   <- perform_mantel_haenszel_test(trial_data, alpha = alpha, trial_type = mh_tt, ni_margin = ni_margin)
...
#  generation_method = "distribution",
#  min_mortality = 0.50, max_mortality = 0.50,   
#  individual_sd = 0.05,
#  mu = 0.1, treatment_sd = 0.00,  # for false positive, set mu = 0
#  recruitment_sd = 0.5,
#  design_type = "superiority",
#  alpha = 0.05
#)

#mean(res_sup$naive_t_test)
#length(res_sup$trial_data[[1]]$sex)
#res_sup$
