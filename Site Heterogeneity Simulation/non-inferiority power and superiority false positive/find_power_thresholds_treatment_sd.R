# trial_parallel.R
# Parallel outer-loop over treatment_sd values for NON-BASKET sims
...
# ---- Helper: run ONE sd value end-to-end (keeps your early-stop) ----
run_one_sd <- function(
    sd_val, total_n, individual_sd, recruitment_sd,
    sim_file,               # which simulator to use (non-inf or super)
    criterion = c("power>0.90", "fp<0.05")  # choose threshold rule
) {
...
# ---- Utility: run a whole scenario in parallel over an sd vector ----
run_scenario <- function(treatment_sd_list, total_n, individual_sd, recruitment_sd,
                         sim_file, criterion, out_file) {
  # Workers = one per sd (bounded by PBS_NCPUS if set)
  n_avail <- as.integer(Sys.getenv("PBS_NCPUS", unset = length(treatment_sd_list)))
  workers <- min(n_avail, length(treatment_sd_list))
  plan(multisession, workers = workers)
  
  message(sprintf(
    "Run: total_n=%d, indiv_sd=%.3f, recruit_sd=%.3f, sim='%s', criterion='%s', workers=%d, |sd|=%d",
    total_n, individual_sd, recruitment_sd, sim_file, criterion, workers, length(treatment_sd_list)
  ))
  
  res_list <- future_lapply(
    treatment_sd_list,
    function(sd)
      run_one_sd(sd_val = sd, total_n = total_n,
                 individual_sd = individual_sd, recruitment_sd = recruitment_sd,
                 sim_file = sim_file, criterion = criterion),
    future.seed = TRUE  # independent, reproducible substreams
  )
...
# ======================
# Define your sd list(s)
# ======================
treatment_sd_list <- c(0.005, 0.01, 0.015, 0.02, 0.025, 0.035, 0.05, 0.075, 0.1)

# ======================
# Non-inferiority block (your original used >0.90 threshold)
# ======================
...
# ======================
# Superiority block (your original used <0.05 threshold)
# ======================
run_scenario(
  treatment_sd_list,
