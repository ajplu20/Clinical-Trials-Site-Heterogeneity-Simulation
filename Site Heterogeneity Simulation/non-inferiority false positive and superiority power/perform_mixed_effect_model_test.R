...
perform_mixed_effect_model_test <- function(df, alpha = 0.05, trial_type = "superiority", basket = FALSE) {
  ...
  if (trial_type == "superiority" && !basket) {
    ...
    # Fit fixed-effects logistic regression
    model <- glmer(death ~ experiment + (1 | site) + syndrome, data = df, family = binomial())
